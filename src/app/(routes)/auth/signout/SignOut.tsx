...
// Determine redirect base dynamically to avoid baking build-time host.
// Prefer runtime window.location.origin; fallback to NEXTAUTH_URL (server) or a safe default.
const runtimeBase =
  (typeof window !== 'undefined' && window.location.origin) ??
  process.env.NEXTAUTH_URL ??
  '';

const signOutURL = `https://${tentantName}.b2clogin.com/${tentantName}.onmicrosoft.com/${userFlow}/oauth2/v2.0/logout?post_logout_redirect_uri=${runtimeBase}`;
  ...
  useEffect(() => {
    ...
    if (status !== 'loading') {
      ...
      // once next-auth doesn't have a session stored, redirect to azure b2c to sign out from the server (and redirect back to the app)
        ...
        window.location.href = signOutURL + (redirectPath ?? '');
      }
    }
